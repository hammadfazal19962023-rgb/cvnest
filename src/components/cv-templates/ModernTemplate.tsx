...
const ModernTemplate = ({ cvData }: { cvData: CVData }) => (
  <div className="space-y-6">
    <div>
      <h3 className="text-2xl font-bold text-gray-900">{cvData.name}</h3>
      <p className="text-amber-600 font-medium">{cvData.title}</p>
      <p className="text-sm text-gray-500 mt-1">{cvData.email} · {cvData.location}</p>
    </div>
    <div className="h-0.5 bg-amber-500 w-16" />
    <div>
    ...
    {cvData.skills?.length > 0 && (
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {cvData.skills.map((s) => (
            <span key={s} className="px-3 py-1 bg-amber-50 text-amber-700 rounded-full text-xs font-medium border border-amber-200">{s}</span>
          ))}
        </div>
      </div>
    )}
