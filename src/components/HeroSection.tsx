...
import { motion, LazyMotion, domAnimation } from "framer-motion";  // ✅ add LazyMotion
...
const HeroSection = () => {
  ...
  const floatProps = (duration = 4) => ({
    animate: { y: [0, -20, 0], rotate: [0, 10, 0] },
    transition: { duration, repeat: Infinity },
  })

  // templates array stays exactly the same...
  const templates = [
    {
      id: "modern",
      name: "Modern",
  ...
  return (
    <LazyMotion features={domAnimation}>  {/* ✅ wrap in LazyMotion */}
      <section
        ...
        {/* Floating emojis */}
        <motion.div className="hidden md:block absolute top-32 left-1/4 text-4xl" {...floatProps(4)}>🚀</motion.div>
        <motion.div className="hidden md:block absolute bottom-40 right-1/4 text-4xl" animate={{ y: [0, -30, 0], rotate: [0, -10, 0] }} transition={{ duration: 5, repeat: Infinity }}>✨</motion.div>
        <motion.div className="hidden md:block absolute top-60 right-20 text-4xl" animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 3, repeat: Infinity }}>💼</motion.div>

        <div className="container relative mx-auto px-4 sm:px-6 lg:px-8 min-h-screen flex items-center">
          <div className="max-w-4xl mx-auto text-center">

            {/* Badge */}
            <motion.div {...badgeProps} className="inline-flex items-center gap-2 bg-white rounded-full px-6 py-3 mb-8 shadow-lg border-2 border-orange-200">
            ...
            {/* Heading */}
            <motion.h1 {...motionProps(0)} className="font-heading text-5xl md:text-7xl font-black mb-6">
              ...
              <br />
              <span className="bg-gradient-to-r from-orange-500 via-pink-500 to-purple-500 bg-clip-text text-transparent hero-gradient-text">
                actually fun
              </span>
            </motion.h1>

            {/* Description */}
            <motion.p {...motionProps(0.1)} className="text-xl text-slate-600 max-w-2xl mx-auto mb-12">
              No boring forms. No judgment. Just a friendly AI that helps you
              brag about yourself (yes, you're that awesome).
            </motion.p>

            {/* Buttons */}
            <motion.div {...motionProps(0.2)} className="flex flex-col sm:flex-row gap-4 justify-center mb-20">
              <Button
                size="lg"
            ...
            {/* Feature cards */}
            <motion.div {...motionProps(0.3)} className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { emoji: "🤖", title: "AI bestie", desc: "Knows what to say when you don't" },
                { emoji: "⚡", title: "Lightning fast", desc: "Because who has time for this?" },
                { emoji: "🎨", title: "Pretty templates", desc: "That actually look good" },
              ].map((feature, i) => (
                <div key={i} className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border-2 border-white">
        ...
        <Dialog open={showTemplates} onOpenChange={setShowTemplates}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-muted-foreground text-center mb-6">Pick a style that fits your personality</p>
